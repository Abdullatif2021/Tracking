...
export const Footer = () => {
  ...
  useEffect(() => {
    ...
    const totalSeconds =
      (parseInt(minutes) * 60 + parseInt(seconds) + parseInt(hours) * 60 * 60 - 1) * 1000
    ...
    <Conatainer style={{minHeight:90}}>
      <Right className='d-flex align-items-center'>
        <CurrentLocationStyle>
          <p className='m-auto textFooterArabic'>الوقت المتوقع لوصول الحافلة</p>
          <p className='m-auto textFooterEnglish pb-1'>The expected time of arrival of the bus</p>
          {/* <p className='m-auto'>5 Minutes, 34 Seconds </p> */}
          {<span className='textFooterResult'>{convertTimeFormat(predectedTime)}</span>}
        </CurrentLocationStyle>
        <Icon>
          <img className='mx-1' src={ArrowLogo}></img>
        </Icon>
        <CurrentLocationStyle>
          <p className='m-auto textFooterArabic'>الوقت الحالي </p>
          <p className='m-auto textFooterEnglish'>Present Time</p>
          {/* <p className='m-auto'>5 Minutes, 34 Seconds </p> */}
          {<span className='textFooterResult'>{arriveTime.toLocaleTimeString([], {hour12: false})}</span>}
        </CurrentLocationStyle>
        <Icon>
          <img className='mx-1' src={ArrowLogo}></img>
        </Icon>
